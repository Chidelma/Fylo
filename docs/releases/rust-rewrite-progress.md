...
| 4 — Native read-only        | Partial                                | Canonical root, link rejection, bounded live/tombstone document and raw-file reads, canonical/custom metadata, Unix xattrs and UID/GID/mode, Windows ADS parser, schema-driven AES-256-GCM/PBKDF2 field decryption with fail-closed errors, bounded validated first-parent commit history plus content-addressed tree/blob hash verification, generation stability, index+WAL scan/reference-integrity verification, predicates, SQL SELECT, inspect/get/get-file/get-deleted/find/scan/verify-index/log/verify-history CLI, no-mutation differential test | Native Windows ADS race-hardening/evidence, full reachable version DAG qualification, exact independent rebuild equivalence, full path/platform matrix, benchmark |
