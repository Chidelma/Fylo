...
| 7 — Machine/CLI replacement | Partial                       | Read-only preview CLI and version identity; `fylo-machine` bounded NDJSON framing with LF delimiting, request/response limits, nested duplicate-key rejection, malformed-frame resume, truncated-frame termination, oversized-frame recovery; thirty-two of the thirty-eight registered operations in published result shapes, covering handshake identity, document/metadata/index/tombstone reads, SQL select and mutation, nested-loop joins, journalled single and bulk writes, restore, commit, and collection create/drop honouring the shared `FYLO_SHARD_WIDTH`; snapshot query pagination with TTID-ascending order, published limits, frame-budget trimming, and `EINVALIDCURSOR` on restart; a kernel-held root lease over `std::fs::File::try_lock`, excluding a live JavaScript owner and being excluded by one, with `EROOTLEASELOST` on a replaced sentinel; non-mutating repository status and diff reporting working-tree cleanliness and per-document changes; `EUNSUPPORTEDOP` for every other registered operation; registry and error-registry conformance harness | The remaining six operations, `schemaMaterialize` upgraders, session root leases, cancellation/signals/pressure, all language clients against exact compiled binaries                                                 |
...
- Rust and JavaScript agree on what an uncommitted working tree changed —
  counts, tree labels, and which documents — and both call a committed tree and
  a commit compared with itself unchanged.
...
- eight joins — every mode, projection, grouping, and identifiers-only —
  ...
  enumeration rather than the contract.
- a collection created by the Rust server is queried and read by the current
  JavaScript engine, survives being re-created with its documents intact,
  refuses a kind contradicting the one on disk, records the shard width both
  engines read from `FYLO_SHARD_WIDTH`, and disappears from JavaScript's view
  once dropped.
...
- every one of the fifteen declared durable transitions was interrupted across
  twelve mutation scenarios, both as a lost process and as a full volume — 180
  interrupted mutations —
  and each root recovered, recovered idempotently, and read back with an index
  matching its documents; a full-volume failure additionally had to leave
  recovery nothing to do.
- one fixed operation log replayed into two cloned roots produces the same
  documents, metadata, query results and counts under both engines, and the
  pinned outcome the log is expected to produce.
